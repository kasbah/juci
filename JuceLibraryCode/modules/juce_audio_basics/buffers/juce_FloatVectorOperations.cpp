...
namespace FloatVectorHelpers
{
    static bool neonPresent = false;

    static bool isNEONAvailable() noexcept
    {
        return true;
        if (neonPresent)
            return true;

        neonPresent = SystemStats::hasNEON();
        return neonPresent;
    }
    ...
    static inline float findMinimumOrMaximum (const float* src, int num, const bool isMinimum) noexcept
    {
        ...
        if (numLongOps > 1 && FloatVectorHelpers::isNEONAvailable())
        {
            float32x4_t val;
...
#define JUCE_BEGIN_NEON_OP \
    if (FloatVectorHelpers::isNEONAvailable()) \
    { \
        const int numLongOps = num / 4;
...
void JUCE_CALLTYPE FloatVectorOperations::copyWithMultiply (float* dest, const float* src, float multiplier, int num) noexcept
{
    ...
    JUCE_PERFORM_NEON_OP_SRC_DEST (dest[i] += src[i],
                                   vmulq_n_f32(s, multiplier),
                                   JUCE_LOAD_SRC)
   #else
    #if JUCE_USE_VDSP_FRAMEWORK
...
void JUCE_CALLTYPE FloatVectorOperations::add (float* dest, float amount, int num) noexcept
{
    ...
    JUCE_PERFORM_NEON_OP_DEST (dest[i] += amount,
                               vaddq_f32 (d, amountToAdd),
                               JUCE_LOAD_DEST)
   #else
    #if JUCE_USE_SSE_INTRINSICS
...
void JUCE_CALLTYPE FloatVectorOperations::multiply (float* dest, float multiplier, int num) noexcept
{
    ...
    JUCE_PERFORM_NEON_OP_DEST (dest[i] *= multiplier,
                                   vmulq_n_f32 (d, multiplier),
                                   JUCE_LOAD_DEST)
   #else
    #if JUCE_USE_VDSP_FRAMEWORK
...
{
   #if JUCE_USE_SSE_INTRINSICS || __ARM_NEON__
